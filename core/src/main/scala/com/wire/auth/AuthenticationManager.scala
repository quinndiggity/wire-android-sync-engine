...
import com.wire.data.JsonDecoder.array
import com.wire.data.{Event, JsonDecoder, UserId}
import com.wire.events.PushNotification
...
object AuthenticationManager {
  ...
  case class Cookie(expiry: Instant, id: Option[Long], tpe: Option[Cookie.Type], label: Option[String], str: Option[String]) {
    val headers = str.map(v => ap(LoginClient.Cookie -> s"zuid=$v")).getOrElse(Map.empty)
    def isValid = expiry.isAfter(Instant.now)
  }

  object Cookie {

    object Type extends Enumeration {
      val session, persistent = Value
    }
    type Type = Type.Value

    def apply(str: String): Cookie = {
      val expiry = str.split('.').toSet.find(_.contains("d=")).map(_.drop(2)).map(v => Instant.ofEpochSecond(v.toLong)).getOrElse(Instant.EPOCH)
      new Cookie(expiry, None, None, None, Some(str))
    }

    implicit lazy val CookieDecoder: JsonDecoder[Cookie] = new JsonDecoder[Cookie] {
      import com.wire.data.JsonDecoder._

      override def apply(implicit js: JSONObject) =
        Cookie('time, decodeOptLong('id), decodeOptString('type).map(Type.withName), decodeOptString('label), None)
    }
  }
}