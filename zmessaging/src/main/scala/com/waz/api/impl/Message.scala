...
class Message(val id: MessageId, var data: MessageData, var likes: IndexedSeq[UserId], var likedBySelf: Boolean)(implicit context: UiModule) extends api.Message with SignalLoading with UiObservable {
  ...
  private def notifyRead() = if (getListenersCount > 0)
    context.zms { zms =>
      if (data.state == api.Message.Status.FAILED || (data.userId != zms.selfUserId && data.state != api.Message.Status.READ)) {
        zms.messages.messageRead(data.id)
      }
    }
